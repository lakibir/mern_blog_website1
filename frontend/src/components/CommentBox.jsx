...
const CommentBox = ({ selectedBlog }) => {
    ...
    useEffect(() => {
        const getAllCommentsOfBlog = async () => {
            try {
                const res = await axios.get(`http://localhost:5000/api/v1/comment/${selectedBlog._id}/comment/all`)
    ...
    const commentHandler = async () => {
        try {
            const res = await axios.post(`http://localhost:5000/api/v1/comment/${selectedBlog._id}/create`, { content }, {
                headers: {
    ...
    const deleteComment = async (commentId) => {
        try {
            const res = await axios.delete(`http://localhost:5000/api/v1/comment/${commentId}/delete`, {
                withCredentials: true
            })
            if (res.data.success) {
    ...
    return (
        <div>
            ...
            {
                comment.length > 0 ? <div className='p-5 bg-gray-100 rounded-md mt-7 dark:bg-gray-800'>
                    {
                                ...
                                <div className='flex items-center justify-between'>
                                    <div className='flex items-start gap-3'>
                                        ...
                                        <div className='mb-2 space-y-1 md:w-[400px]'>
                                            <h1 className='font-semibold'>{item?.userId?.firstName} {item?.userId?.lastName} <span className='ml-2 text-sm font-light'>yesterday</span></h1>
