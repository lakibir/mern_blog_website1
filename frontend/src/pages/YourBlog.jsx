...
const YourBlog = () => {
    ...
    const getOwnBlog = async () => {
        try {
            const res = await axios.get(`https://mern-blog-website1-tpd1.onrender.com/api/v1//blog/get-own-blogs`, { withCredentials: true })
            if (res.data.success) {
    ...
    const deleteBlog = async (id) => {
        try {
            const res = await axios.delete(`https://mern-blog-website1-tpd1.onrender.com/api/v1//blog/delete/${id}`, { withCredentials: true })
            if (res.data.success) {
                            ...
                            {blog?.map((item, index) => (
                                <TableRow key={index}>
                                    ...
                                    <TableCell className="flex items-center gap-4">
                                        <img src={item.thumbnail} alt="" className='hidden w-20 rounded-md md:block' />
                                        <h1 className='cursor-pointer hover:underline' onClick={() => navigate(`/blogs/${item._id}`)}>{item.title}</h1>
                                    </TableCell>
